...
int main(int argc, char *argv[])
{
	...
	sockfd = socket(AF_INET, SOCK_STREAM, 0); //sockfd este socketul pe care se asculta conexiuni
	DIE(sockfd < 0, "socket");
			...
			if (FD_ISSET(i, &tmp_fds)) { 
				if (i == sockfd) {
					...
					if (n == 0) {
						...
						sprintf(notification, "[UPDATE] Client de pe socketul %d a inchis conexiunea\n", i);
