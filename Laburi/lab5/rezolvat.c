...
void ping(int sockfd, struct in_addr ip_addr, int count)
{
	...
	while(i < count || count == -1) {
		...
		*/
		...
		ip_hdr->ihl = 5; // dimensiunea headerului in cuvinte de 32 de biti
		ip_hdr->ttl = 64;
		ip_hdr->protocol = IPPROTO_ICMP;
		ip_hdr->id = htons(getpid() & 0xFFFF); // sau getpid()
		...
		*/
		...
		printf("%ld bytes from %s: icmp_seq=%hu ttl=%u time=%.1f\n", pkt.size,
