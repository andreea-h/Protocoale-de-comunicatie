...
void ping(int sockfd, struct in_addr ip_addr, int count)
{
	...
	/*while(i <= count || (count == -1))*/ 
		...
		ip_hdr->id = htons(getpid()); //pid-ul procesului
	    ip_hdr->daddr = ip_addr.s_addr; //destination address - address in network byte order
	    									//(uint32_t)

	    ip_hdr->tot_len = htons(sizeof(struct iphdr) + sizeof(struct icmphdr));
	    //se seteaza ip_hdr->saddr egala cu adresa ip ascioata interfetei noastre IFNAME
	    get_interface_ip(sockfd, IFNAME, &ip_hdr->saddr);

	    //calculeaza checksum-ul /// !!!ultimul completat
	    ip_hdr->check = 0;
		...
		/* TODO 1 afișați informațiile relevante din pachetul reply */
			//ne uitam la pachetul intors in antetul rip_hdr
...
void traceroute(int sockfd, struct in_addr ip_addr)
{
	...
	for(i = 0; i < 32; i++) {
		...
		ip_hdr->ihl = 5; //lungimea headerului (5 cuvinte de 32 de biti fiecare)
		                 // imi arata cate cuvinte(de 32 biti fiecare) se citesc mai departe pt header
		ip_hdr->ttl = i;  //seteaza ttl-ul incremental
		ip_hdr->protocol = IPPROTO_ICMP; //tipul de protocol
		...
		char* next_ip_addr = inet_ntoa(a);
	
		printf("%d. reply from: %s time=%.1fms\n",i + 1, next_ip_addr, time_elapsed);

		int response_type = ricmp_hdr->type;
		if(response_type == 0) { //destinatia raspunde cu type = 0
			break;
		}
	}
